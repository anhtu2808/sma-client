...
export const SkillSelector = ({ titleClassName, itemClassName, groupClassName }) => {
                    ...
                    {Object.entries(groupedSkills).map(([group, skills]) => (
                        <div key={group} className="flex flex-col gap-2">
                            ...
                            <div className="flex flex-wrap gap-2.5">
                                {skills.map((skill) => (
                                    <div key={skill.skillId} className={`inline-flex items-center w-fit justify-between gap-2 px-3 py-1.5 rounded-md text-[13px] font-medium group/skill transition-colors ${itemClassName || 'bg-transparent text-gray-800 border border-gray-200'}`}>
