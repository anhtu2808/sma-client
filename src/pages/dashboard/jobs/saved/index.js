...
const SavedJobs = ({ jobs, isLoading, isError }) => {
  if (isLoading) {
    return (
  ...
  if (isError) {
    return (
      <div className="text-center py-16 rounded-2xl bg-white border border-gray-100">
        ...
        <h3 className="mt-3 text-xl font-bold text-gray-800">Failed to load saved jobs</h3>
  ...
  return (
    <div className="space-y-5">
      {jobs.map((job) => (
        <DashboardJobCard activeTab="saved" job={job} key={job.id} />
      ))}
    </div>
  );
};
