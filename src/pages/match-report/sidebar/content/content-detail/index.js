...
const ContentDetail = ({ item, siblingDetailIds, isLast }) => {
  const dispatch = useDispatch();
  const { editor } = useContext(EditorContext);
  const expandedItemIds = useSelector((state) => state.matchingReport.ui.expandedItemIds);
  const isFocused = useSelector(
    (state) => state.matchingReport.ui.focusedItemId === item.id
  );
  ...
  const idsToMark = siblingDetailIds && siblingDetailIds.length > 0 ? siblingDetailIds : [item.id];

  const hasContextInEditor = useMemo(() => {
    if (!editor || !item.context || editor.isDestroyed) return false;
    try {
      return findTextInDoc(editor.state.doc, item.context).length > 0;
    } catch {
      return false;
    }
  }, [editor, item.context, editor?.state?.doc]);
  ...
  const isPositive =
    ...
    item.status === "MATCHED" ||
    item.status === "matched";
  const statusConfig = getStatusConfig(isPositive ? "MATCHED" : item.status);
  const hasSuggestions = Array.isArray(item.suggestions) && item.suggestions.length > 0;
  const isExpanded = expandedItemIds.includes(item.id);
  const canMarkAsFixed = hasSuggestions && !item.isFixed && !isMarkingFixed;
  const canRegenerate = !item.isFixed;
    ...
    <>
      ...
      <div className="flex shrink-0 items-center gap-3">
        {hasSuggestions && isExpanded && !hasContextInEditor && (
          ...
          >
            <FontAwesomeIcon
              icon={isMarkingFixed ? faArrowsRotate : item.isFixed ? faCircleCheck : faCirclePlus}
  ...
  return (
    <div
      ...
      className={`transition-all duration-500 ${isLast ? "" : "border-b border-neutral-200"} ${
        isFocused ? focusStyles.bg : item.isFixed ? "bg-emerald-50/40" : "bg-white"
      }`}
    >
      <button
        type="button"
        onClick={() => {
          ...
          if (willExpand) {
            ...
            dispatch(setHighlightModalDetailId(item.id));
          }
        }}
