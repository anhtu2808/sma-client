...
const Projects = ({ data, onChange }) => {
  ...
  const addBullet = (itemIndex) => {
    const newData = [...data.items];
    newData[itemIndex].bullets.push({ label: 'New Detail', value: 'Description' });
    onChange('items', newData);
  };
  ...
  const addItem = () => {
    onChange('items', [
      ...
      { 
        ...
        bullets: [{ label: 'Feature', value: 'Cool stuff' }] 
      }
    ]);
  };
            ...
            <ul className="list-disc pl-6 text-[13px] text-[#333] space-y-1">
              {item.bullets.map((bullet, bIdx) => (
                <li key={bIdx} className="relative group/bullet ml-1">
                  ...
                  {/* Nested lists or rich text work well here inside value */}
                  <EditableField 
                    html={bullet.value} 
                    onChange={(e) => handleBulletChange(index, bIdx, 'value', e.target.value)}
                    tagName="span"
                    className="inline-block align-top ml-1 max-w-[90%]"
                  />
