...
const CTAButtons = () => {
    ...
    const [isCheckMatchModalOpen, setIsCheckMatchModalOpen] = useState(false);
    ...
    const handleOpenCheckMatch = () => {
        const isAuthorized = requireLogin({
            warningMessage: "Please log in to check AI matching score.",
            redirectTo: `/jobs/${id}`,
        });

        if (isAuthorized) {
            setIsCheckMatchModalOpen(true);
        }
    };
    ...
    return (
        <>
            ...
            <CheckMatchModal
                open={isCheckMatchModalOpen}
                onClose={() => setIsCheckMatchModalOpen(false)}
                jobId={id}
